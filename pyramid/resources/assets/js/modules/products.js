const Products = {
    ...
    index(options = {}) {
        ...
        table.on('click', '.action-edit', function (e) {
            ...
            window.location.href = `${Products.baseUrl()}/${value}/edit`;
        });
        ...
        table.on('change', '.menu-status-switch', function (e) {
            ...
            Utils.ajax(`${Products.baseUrl()}/toggle-status/${value}?status=${checked}`)
                .then(() => Utils.notify('', 'Update status produk berhasil'))
                .catch(() => Utils.notify('', 'Gagal memperbarui status produk'));
        })
    ...
    loadTable(options) {
        ...
        let datatableButtons = `
            <div class="dataTables_buttons actions">
                ...
                <span class="actions__item zmdi zmdi-cloud-upload" data-table-action="import" title="" data-toggle="tooltip" data-placement="top" data-original-title="Import Produk"></span>
            </div>
        `
        ...
        $('body').on('click', '[data-table-action]', function (e) {
            ...
            if (action === 'create') {
                window.location.href = `${Products.baseUrl()}/create`;
            }
            if (action === 'reload') {
    ...
    delete(id) {
        Swal({
            ...
            preConfirm: (login) => {
                return fetch(`${Products.baseUrl()}/${id}`, {
                    method: 'DELETE',
    ...
    import() {
        ...
        body.html(`
            <form action="">
                <div class="form-group">
                    <label>Store</label>
                    <select class="select2" name="store">${options}</select>
                </div>
                <div class="form-group">
        ...
        body.on('submit', 'form', function (e) {
            ...
            Utils.ajaxFile('/manage/products/import', 'POST', 'json', formData)
                .fail(xhr => {
