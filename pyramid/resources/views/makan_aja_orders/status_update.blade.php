...
<form action="{{ url('manage/makan-aja/update/' . $order->id . '?action=status-update') }}" method="POST">
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="form-group required">
                ...
                <select class="select2 form-control" name="merchant_paid_by">
                    <option {{ $order->merchant_paid_by == 'Admin' ? 'selected' : '' }} value="Admin">Admin</option>
                    <option {{ $order->merchant_paid_by == 'Driver' ? 'selected' : '' }} value="Driver">Driver</option>
                </select>
                <i class="form-group__bar"></i>
            </div>
            <div class="form-group required">
                ...
                <select class="select form-control" name="payment_method">
                    @foreach(OrderPaymentMethod::cases() as $method)
                        ...
                        >{{ MakanAjaOrderUtil::getPaymentMethodText($method) }}</option>
                    @endforeach
                </select>
                <i class="form-group__bar"></i>
            </div>
            <div class="form-group required">
                ...
                <select class="select form-control" name="status">
                    ...
                    @if($order->status < 8)
                        <option value="8">{{ MakanAjaOrderUtil::getStatusText(MakanAjaOrderStatus::from(8)) }}</option>
                    @else
                        <option value="9">{{ MakanAjaOrderUtil::getStatusText(MakanAjaOrderStatus::from(9)) }}</option>
                    @endif
                </select>
                <i class="form-group__bar"></i>
            </div>
