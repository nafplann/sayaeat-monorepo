...
@section('scripts')
    <script>
                            ...
                            return `
                                <div class="d-flex align-content-center justify-content-center">
                                    <div class="toggle-switch">
                                        <input type="checkbox" class="toggle-switch__checkbox merchant-status-switch" value="${row.id}" ${row.status === 'OPEN' ? 'checked' : ''}>
        ...
        $('.base-datatable').on('change', '.merchant-status-switch', function (e) {
            e.preventDefault();
            let value = $(this).val();
            let checked = $(this).is(':checked');

            Utils.ajax(`${Base.baseUrl()}/toggle-status/${value}?status=${checked}`)
                .then(() => Utils.notify('', 'Update status merchant berhasil'))
                .catch(() => Utils.notify('', 'Gagal memperbarui merchant'));
        })
    </script>
@endsection
