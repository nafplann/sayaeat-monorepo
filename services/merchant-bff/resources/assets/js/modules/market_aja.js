const MarketAja = {
    baseUrl() {
        return $('.market-aja-datatable').data('url');
    },
    /**
     * @param options
     ...
     */
    index(options = {}) {
        MarketAja.loadTable(options);

        let table = $('.market-aja-datatable');

        table.on('click', '.action-process', function (e) {
            e.preventDefault();
            let value = $(this).data('id');
            MarketAja.process(value);
        });

        table.on('click', '.action-details', function (e) {
            e.preventDefault();
            let value = $(this).data('id');
            MarketAja.details(value);
        });

        table.on('click', '.action-cancel', function (e) {
            e.preventDefault();
            let value = $(this).data('id');
            MarketAja.cancel(value);
        });

        $('#market-settings-button').on('click', function (e) {
            MarketAja.settings();
        });
        ...
        if (options.tableAutoRefresh) {
            ...
            setInterval(() => {
                MarketAja.reloadTable();
            }, 15000);
        }
    },
    loadTable(options) {
        let statusCategory = options.statusCategory ?? '';
        let columns = $('.market-aja-datatable').find('thead th').map(function () {
            let row = $(this);
        ...
        `
        this.table = Utils.datatable('.market-aja-datatable', {
            ajax: {
    ...
    process(orderId) {
        ...
        Utils.ajax(Utils.baseUrl(`manage/market-aja/process/${orderId}`), 'GET', 'application/json', {}, {
        ...
        $(body).on('blur', '[name="distance"]', function (e) {
            ...
            Utils.ajax(Utils.baseUrl('manage/market-aja/calculate-fees'), 'POST', 'application/json', {
                subtotal, distance
            }, {
                success: function (response) {
    ...
    details(orderId) {
        ...
        Utils.ajax(Utils.baseUrl(`manage/market-aja/details/${orderId}`), 'GET', 'application/json', {}, {
            success: function (response) {
                body.html(response);
            }
        });
    ...
    cancel(orderId) {
        Swal({
            ...
            preConfirm: (reason) => {
                Utils.ajax(Utils.baseUrl(`manage/market-aja/cancel/${orderId}?reason=${reason}`), 'POST', 'application/json', {}, {
                    success: function (response) {
                        if (response.status) {
                            Utils.notify('', response.message);
                            MarketAja.reloadTable();
    ...
    settings() {
        ...
        Utils.ajax(Utils.baseUrl(`manage/market-aja/settings`), 'GET', 'application/json', {}, {
            success: function (response) {
                body.html(response);
            }
        });
