const KirimAja = {
    baseUrl() {
        return $('.kirim-aja-datatable').data('url');
    },
    /**
     * @param options
     ...
     */
    index(options = {}) {
        KirimAja.loadTable(options);

        let table = $('.kirim-aja-datatable');

        table.on('click', '.action-process', function (e) {
            e.preventDefault();
            let value = $(this).data('id');
            KirimAja.process(value);
        });

        table.on('click', '.action-details', function (e) {
            e.preventDefault();
            let value = $(this).data('id');
            KirimAja.details(value);
        });

        table.on('click', '.action-cancel', function (e) {
            e.preventDefault();
            let value = $(this).data('id');
            KirimAja.cancel(value);
        });

        $('#kirim-settings-button').on('click', function (e) {
            KirimAja.settings();
        });
        ...
        if (options.tableAutoRefresh) {
            ...
            setInterval(() => {
                KirimAja.reloadTable();
            }, 15000);
        }
    },
    loadTable(options) {
        let statusCategory = options.statusCategory ?? '';
        let columns = $('.kirim-aja-datatable').find('thead th').map(function () {
            let row = $(this);
            ...
            if (column === '_action') {
                return {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        let actionableStatusCode = [2, 3, 4, 5, 6];
        ...
        `
        this.table = Utils.datatable('.kirim-aja-datatable', {
            ajax: {
            ...
            createdRow: function (row, data, dataIndex) {
                let actionableStatus = [2, 3, 4, 5, 6];
    ...
    process(orderId) {
        ...
        Utils.ajax(Utils.baseUrl(`manage/kirim-aja/process/${orderId}`), 'GET', 'application/json', {}, {
        ...
        $(body).on('blur', '[name="distance"]', function (e) {
            ...
            Utils.ajax(Utils.baseUrl('manage/kirim-aja/calculate-fees'), 'POST', 'application/json', {
                subtotal, distance
            }, {
                success: function (response) {
    ...
    details(orderId) {
        ...
        Utils.ajax(Utils.baseUrl(`manage/kirim-aja/details/${orderId}`), 'GET', 'application/json', {}, {
            success: function (response) {
                body.html(response);

                let senderAddress = body.find('#sender-address').text();
                let senderLatitude = body.find('#sender-latitude').text();
                let senderLongitude = body.find('#sender-longitude').text();
                ...
                let marker1 = new mapboxgl.Marker({color: 'green'})
                    .setLngLat([senderLongitude, senderLatitude])
                    .setPopup(popup)
                    .addTo(map);

                body.find('.recipient-id').each(function (index, element) {
                    let recipientId = $(this).text();
                    let recipientAddress = body.find('.recipient-address').eq(index).text()
                    let recipientLatitude = body.find('.recipient-latitude').eq(index).text();
                    let recipientLongitude = body.find('.recipient-longitude').eq(index).text();
                    ...
                    new mapboxgl.Marker({color: 'blue'})
                        .setLngLat([recipientLongitude, recipientLatitude])
    ...
    cancel(orderId) {
        Swal({
            ...
            preConfirm: (reason) => {
                Utils.ajax(Utils.baseUrl(`manage/kirim-aja/cancel/${orderId}?reason=${reason}`), 'POST', 'application/json', {}, {
                    success: function (response) {
                        if (response.status) {
                            Utils.notify('', response.message);
                            KirimAja.reloadTable();
    ...
    settings() {
        ...
        Utils.ajax(Utils.baseUrl(`manage/kirim-aja/settings`), 'GET', 'application/json', {}, {
            success: function (response) {
                body.html(response);
            }
        });
