...
class Store extends AuditableModel
{
    ...
    public function scopeHavingProducts(Builder $query, ?string $term): void
    {
        if (!$term) {
            $query->with('products', function ($q) use ($term) {
                $q->where('status', 1)
                    ->inRandomOrder()
                    ->limit(2);
            });
        ...
        $query->where('name', 'LIKE', "%$term%")
            ->orWhereHas('products', function ($q) use ($term) {
                $q->where('status', 1)
                    ->where('name', 'LIKE', "%$term%");
            })
            ->with('products', function ($q) use ($term) {
                $q->where('status', 1)
                    ->where('name', 'LIKE', "%$term%")
                    ->limit(3);
            });
