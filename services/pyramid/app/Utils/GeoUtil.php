...
/*
 * https://github.com/ericbenedetto16/random-lat-lng
 * */
class GeoUtil
{
    /**
     ...
     */
    public static function randomizeLocation($lat, $lng, $radiusConstraintLower, $radiusConstraintUpper)
    {
        // Randomize Lat Lng
        $maxDegLat = ($radiusConstraintUpper / 69.172) * 1000;
        $maxDegLng = ($radiusConstraintUpper / self::getLngDegree($lat, $lng) ) * 1000;
        ...
        $dist = self::haversine($lat, $lng, $rLat, $rLng);
        ...
        if ($dist >= $radiusConstraintLower && $dist <= $radiusConstraintUpper) {
            ...
            while ($dist < $radiusConstraintLower || $dist > $radiusConstraintUpper) {
                $maxDegLat = ($radiusConstraintUpper / 69.172) * 1000;
                $maxDegLng = ($radiusConstraintUpper / self::getLngDegree($lat) ) * 1000;
                $rLat = $lat + rand(-$maxDegLat, $maxDegLat)/1000;
                $rLng = $lng + rand(-$maxDegLng, $maxDegLng)/1000;
     ...
     */
    private static function getLngDegree($lat)
    {
