...
<div class="invoice pt-0">
    @if($order->status === MakanAjaOrderStatus::CANCELED->value)
    ...
    <div class="row invoice__attrs">
        ...
        <div class="col">
            <div class="invoice__attrs__item">
                <small>Status</small>
                <h3>{{ MakanAjaOrderUtil::getStatusText(MakanAjaOrderStatus::from($order->status)) }}</h3>
            </div>
        </div>
        ...
        <div class="col">
            <div class="invoice__attrs__item">
                ...
                <h3>{{ MakanAjaOrderUtil::getPaymentMethodText(OrderPaymentMethod::from($order->payment_method)) }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="invoice__remarks">
        <div class="row">
            ...
            <div class="col-md-4">
                <h5 class="mt-3">Merchant</h5>
                <p>{{ $order->merchant->name }}</p>

                <h5>Bank Name</h5>
                <p>{{ $order->merchant->bank_name ?? '-' }}</p>

                <h5>Bank Account Number</h5>
                <p>{{ $order->merchant->bank_account_number ?? '-' }}
                    an {{ $order->merchant->bank_account_holder ?? '-' }}</p>

                <h5>QRIS Link</h5>
                <p>{{ $order->merchant->qris_link ?? '-' }}</p>
            </div>
            <div class="col-md-4">
            ...
            @foreach($order->items as $item)
                <tr>
                    <td style="width: 50%">
                        ...
                        @if ($item->addons)
                            <br>
                            <small>Addons: {{ $item->addons }}</small>
                        @endif
                        @if ($item->remark)
                            <br>
